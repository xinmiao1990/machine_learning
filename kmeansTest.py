...
def transform_image(image, code_vectors):
    ...
    print(image.shape)
    print('shape of the code_vectors:')
    print(code_vectors.shape)
    n_vec = code_vectors.shape[0]
    N1, N2, D = image.shape
    dis_euc = np.tile(0.0, (N1, N2, n_vec))

    for k in range(0, n_vec):
        temp = np.tile(code_vectors[k, :], (N1, N2, 1))
        assert temp.shape == (N1, N2, D)
        assert code_vectors[k, :].shape == (D,)
        diff = image - np.tile(code_vectors[k, :], (N1, N2, 1))
        dis_euc[:, :, k] = np.sum(diff * diff, axis=2)
    member = np.argmin(dis_euc, axis=2)
    temp1 = code_vectors[member.flatten(0), :].T
    temp1 = np.reshape(temp1, (D, N1, N2))
    new_im = np.transpose(temp1, (1, 2, 0))
    return new_im
...
def kmeans_image_compression():
    ...
    k_means = KMeans(n_cluster=16, max_iter=2, e=1e-6)
